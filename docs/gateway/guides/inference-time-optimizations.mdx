...
<Note>
  These strategies are implemented as variant types with an `experimental_` name prefix.
  They are fully supported, but their configuration format, behavior, and names may change in
  backwards-incompatible ways between releases as we refine them based on user feedback.
</Note>
...
To use BoN sampling in TensorZero, you need to configure a variant with the `experimental_best_of_n_sampling` type.
...
Read more about the `experimental_best_of_n_sampling` variant type in [Configuration Reference](/gateway/configuration-reference).
