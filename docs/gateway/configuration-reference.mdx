...
| `experimental_best_of_n_sampling`          | Generates multiple response candidates with other variants, and selects the best one using an evaluator model.                                                                  |
...
<Note>
  Variant types prefixed with `experimental_` are fully supported, but they are still evolving:
  their configuration format, behavior, and even their names may change in backwards-incompatible
  ways between releases as we refine them based on user feedback. If you rely on an experimental
  variant type, pin your gateway version and review the release notes before upgrading.

  The `experimental_chain_of_thought` variant type is deprecated: use `chat_completion` with a
...
<Accordion title='type: "experimental_best_of_n_sampling"'>
